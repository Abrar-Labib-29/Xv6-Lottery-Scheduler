...
// reset tickets for all processes
...
// Check if all runnable processes have 0 tickets
int all_tickets_used(void)
{
  struct proc *p;
  for (p = proc; p < &proc[NPROC]; p++)
  {
...
found:
  p->pid = allocpid();
  p->state = USED;
  init_tickets(p); // initializing tickets
...
// modified scheduler to implement lottery scheduling
